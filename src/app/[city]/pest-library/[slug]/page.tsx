...
function smartPluralizeSentence(
  text: string,
  singularName: string,
  pluralName: string
) {
  ...
  if (singularName.trim()) {
    result = result.replace(
      new RegExp(`\\b${singularEscaped}\\b`, "gi"),
      pluralName
    );
  }
...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  const canonical = `https://www.allstarpestkc.com/${cityData.slug}/pest-library/${pest.slug}`;
    ...
    "@graph": [
      {
        ...
        itemListElement: [
          ...
          },
          {
            ...
            position: 4,
            name: pluralName,
            item: `https://www.allstarpestkc.com/${cityData.slug}/pest-library/${pest.slug}`,
          },
        ],
      },
      {
      ...
      <section className="mx-auto max-w-7xl px-6 py-12 lg:px-8">
        <div className="grid gap-8 lg:grid-cols-[1.15fr_0.85fr]">
          <div className="rounded-3xl border border-zinc-200 bg-white p-8 shadow-sm">
            ...
            {!beneficial && (
              <p className="mt-4 text-base leading-8 text-zinc-700">
                In {cityData.name}, {cityData.state}, seasonal activity, moisture,
                food sources, landscaping, crawl spaces, garages, attics, and
                structural gaps can all contribute to recurring pest pressure.
                Proper identification and a targeted plan make a big difference.
              </p>
            )}

            {beneficial && (
              <p className="mt-4 text-base leading-8 text-zinc-700">
                When {pluralName.toLowerCase()} are active near a home in {cityData.name},
                it is important to separate beneficial pollinator activity from a true
                structural pest issue. Safe relocation is usually the best path when
                human contact is the concern.
              </p>
            )}
          </div>
          ...
          <div className="rounded-3xl border border-red-200 bg-gradient-to-br from-red-50 via-white to-yellow-50 p-8 shadow-sm">
            ...
            <p className="mt-4 text-base leading-8 text-zinc-700">
              All Star Pest Solutions proudly serves {cityData.name}, {cityData.state}
              and the surrounding Kansas City metro with safe, modern, effective
              pest solutions.
            </p>
      ...
      <section className="mx-auto max-w-7xl px-6 pb-12 lg:px-8">
        <div className="rounded-3xl border border-zinc-200 bg-zinc-50 p-8">
          ...
          <p className="mt-4 max-w-4xl text-base leading-8 text-zinc-700">
            Looking for information about {pluralName.toLowerCase()} in nearby areas?
            Browse city-specific pages below to find the same pest in surrounding
            service areas.
          </p>
